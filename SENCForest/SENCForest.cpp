...
SENCForest::SENCForest( data & dataObject): _dataObject(dataObject){}

SENCForest::SENCForest( data & dataObject, int numSENCTrees, int sampleSize, int minSizeOfNode):_dataObject(dataObject), _numSENCTrees(numSENCTrees), _sampleSize(sampleSize), _minSizeOfNode(minSizeOfNode){
...
void SENCForest::deploySENCForest(int sampleSize, data & refdeltaD)
{
	vector<pair<int, vector<treenode *>>> newClass;
	int i;
	for( i=0; i<refdeltaD.dataVector.size(); i++)
	{
		vector<int> fre(_numberOfClasses+1, 0);
		vector<treenode *> nodes;
		for(int treeId = 0; treeId < _numSENCTrees; treeId++)
		{
			fre[_SENCTrees[treeId]->getLabel(i, nodes,refdeltaD)]++;
		}
	...
	constructINCSENCForest(newClass, sampleSize,refdeltaD);
}

void SENCForest::constructINCSENCForest(vector<pair<int, vector<treenode *>>>& newClass, int sampleSize, data & refdeltaD){
		...
		for(int treeId = 0; treeId < _numSENCTrees; treeId++){
			// cout<<treeId<<endl;
			_SENCTrees[treeId]->insertUpdatesInSENCTree(newClass, sampleSize, treeId,refdeltaD);
		}
}
...
long double SENCForest::computeAnomalyScore(int pointX,  data & testDataObject){
...
long double SENCForest::computeAvgPathLength(int pointX,  data & testDataObject){
...
long double SENCForest::computeRelativeMass(int pointX,  data & testDataObject)
{
