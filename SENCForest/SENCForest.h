...
class SENCForest
{
    public:
	//SENCForest();
    SENCForest( data &);                              //used for: create an empty forest followed by reading the history in blank forest
	SENCForest( data &, int, int, int);                    //used for: create a forest and initialize _sampleSize and _numSENCTrees
    virtual ~SENCForest();
    void constructSENCForest(int);    						//used for creating an SENCForest (static vesion) 
    void constructINCSENCForest(vector<pair<int, vector<treenode *>>> &, int, data &);      				//used for: create an i^2Forest (incremental SENCForest version of SENCForest)
    void writeFOREST(const string &);					//writes history in file.
    void readFOREST(const string &);        			//reads history from file.
	long double computeAnomalyScore(int,  data &);		        //compute anomaly score of a point passed as an int argument
	long double computeRelativeMass(int,  data &);		        //compute anomaly score of a point passed as an int argument
	void deploySENCForest(int, data &);

	private:    
	long double computeAvgPathLength(int,  data &);              //compute average path length of point passed as an argumnet i.e Averaged across all the SENCTrees
	...
	int _avgPLEstimationOfBST;							//average path length estimation of BST having '_sampleSize' number of nodes. 
	 data & _dataObject;							//reference of the input dataObject, only a container not responsible for deletion of the object.
