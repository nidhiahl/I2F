...
void iforest::constructINCiForest(int updateSampleSize,data &refdeltaD){
	...
	int newdata_numInstances = refdeltaD.getnumInstances();
	int newSampleSize = int((_sampleSize/_numInstances)*(_numInstances+newdata_numInstances));
	//_sampleSize = (_smapleSize/_numInstances)*(_numInstances+newdata_numInstances)
	if (newSampleSize=_sampleSize){
		
	}else{
		updateSampleSize=newSampleSize-_sampleSize;
