...
SENCTree::SENCTree(const data & dataObject): _dataObject(dataObject){}

SENCTree::SENCTree(const data & dataObject, int sampleSize, int maxTreeHeight, int maxNumOfNodes, int avgPLEstimation, int minSizeOfNode): _dataObject(dataObject), _sampleSize(sampleSize), _maxTreeHeight(maxTreeHeight), _maxNumOfNodes(maxNumOfNodes), _minSizeOfNode(minSizeOfNode){}
...
//*******************************************************Static EValuation*****************************************************************//
int SENCTree::getLabel(int pointX, vector<treenode *>& newClass)
{
	...
	if(getRadius(_dataObject.dataVector[pointX]->attributes, node->centroid) > node->radius)
	{	
...
long double SENCTree::computePathLength(int pointX, const data & testDataObject){
...
long double SENCTree::computeRelativeMass(int pointX, const data & testDataObject){
...
void SENCTree::insertUpdatesInSENCTree(vector<pair<int, vector<treenode *>>>& indices, int sampleSize, int treeId){
	...
	for(int i=0; i<numLeafNodes; i++)
	{
		if(leafNodes[i]->updateIndices.size()>0)
		{
			...
			reconstructSENCTree(leafNodes[i], pseudoPoints);
...
void SENCTree::reconstructSENCTree(treenode * rootNode, int pseudoPoints){
...
// 				//Creating Ball
				...
				for(int i=0; i<currNode->updateIndices.size(); i++)
				{
					for(int j=0;j<_dataObject.getnumAttributes();j++)
					{
						currNode->centroid[j]+=(_dataObject.dataVector[currNode->updateIndices[i]]->attributes[j] - currNode->centroid[j])/(i+1);
					}
				}
				...
				for(int i=0;i<currNode->updateIndices.size();i++)
				{
					currNode->radius=max(currNode->radius, getRadius(_dataObject.dataVector[currNode->updateIndices[i]]->attributes, currNode->centroid));
				}
				if(tmp.second)
				{
...
// 				//Maintain frequency
    			...
    			currNode->splitValue = currNode->splitInfoSelectionForUpdates(_dataObject, rootNode->centroid, tmp.second);
    			currNode->createLeftChild();
				currNode->createRightChild();
				for(int i=0; i<currNode->updateIndices.size(); i++){     
