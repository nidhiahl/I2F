...
class PCB_EIF_Window():
    ...
    def __init__(self, dimension, window_size: int = 256, num_trees: int = 100, anomaly_threshold: float = 0.5,
        ...
        self.update_time = 0
        self.update_init_time = 0
        self.update_temp_stop_time = 0
...
#run_train_classifier definition when D0 is parsed fully and remaining data is parsed only using windows
        ...
        if len(input) == 0:
            ...
            if not self.init_flag:
                ...
                if len(self.initial_data) == self.initial_data_size:
                    ...
                    self.update_init_time = time.time()
            else:
                ...
                #print("window_growing",len(self.window),self.window_size,self.n_samples_count % self.window_size,self.update_data_size)
                ...
                if (self.n_samples_count % self.window_size == 0 and self.n_samples_count !=0):
                    ...
                    if self.ndkswindow.detected_change():
                        ...
                        self.update_temp_stop_time  = time.time()
                        #print("self.update_temp_stop_time",self.update_temp_stop_time)
                        self.update_time += self.update_temp_stop_time -self.update_init_time
                        #print("self.update_time",self.update_time)
                        for point in self.window:
                            #print("before score computation")
                            overall_ascore, individual_ascore = self.F.compute_paths(point) # self.window[-1]
                            #print(overall_ascore)
                            self.ascore.append(overall_ascore)
                        self.update_init_time = time.time()
                        #print("self.update_init_time",self.update_init_time)
                        #print("model update non-last window")
                    else:
                        self.prev_update_n_samples_count = self.n_samples_count
                        self.update_temp_stop_time  = time.time()
                        #print("self.update_temp_stop_time",self.update_temp_stop_time)
                        self.update_time += self.update_temp_stop_time -self.update_init_time
                        #print("self.update_temp_stop_time -self.update_init_time",self.update_temp_stop_time -self.update_init_time)
                        #print("self.update_time",self.update_time)
                        for point in self.window:
                            #print("before score computation")
                            overall_ascore, individual_ascore = self.F.compute_paths(point) # self.window[-1]
                        ...
                        self.update_init_time = time.time()
                        #print("self.update_init_time",self.update_init_time)
                        #print("model not updated for non-last window")
                elif self.n_samples_count == self.update_data_size:
                    #print("last window",self.window_size,self.n_samples_count % self.window_size,self.update_data_size,self.n_samples_count)
                    self.ndkswindow.add_element(np.asarray(self.window))
                    if self.ndkswindow.detected_change():
                        self.update_counter += 1
                        #print("change detected",len(self.window[-(self.update_data_size - self.prev_update_n_samples_count):]))
                        self.F.update_model(window=self.window)
                        self.update_temp_stop_time  = time.time()
                        #print("self.update_temp_stop_time",self.update_temp_stop_time)
                        self.update_time += self.update_temp_stop_time -self.update_init_time
                        #print("self.update_time",self.update_time)
                        for point in self.window[-(self.update_data_size - self.prev_update_n_samples_count):]:
                            #print("before score computation")
                            overall_ascore, individual_ascore = self.F.compute_paths(point) # self.window[-1]
                            #print(overall_ascore)
                            self.ascore.append(overall_ascore)
                        self.update_init_time = time.time()
                        #print("self.update_init_time",self.update_init_time)
                        #print("model update last window")
                    else:
                        #print("change not setected", len(self.window[-(self.update_data_size - self.prev_update_n_samples_count):]))
                        self.update_temp_stop_time  = time.time()
                        #print("self.update_temp_stop_time",self.update_temp_stop_time)
                        self.update_time += self.update_temp_stop_time -self.update_init_time
                        #print("self.update_time",self.update_time)
                        ...
                        self.update_init_time = time.time()
                        #print("self.update_init_time",self.update_init_time)
                        #print("model not updated for last window")
                        
    def give_update_time(self):
    ...
    def compute_ascore(self, input:np.ndarray):
        overall_anomaly_score, individual_anomaly_score = self.F.compute_paths(input) # self.window[-1]
        return overall_anomaly_score
