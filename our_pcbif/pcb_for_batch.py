...
D0_path = "../io_directory/D0.csv"
deltaD_path = "../io_directory/deltaD1.csv"
...
for i in range(num_iteration):
    ...
    ix = np.random.choice(D0_size, size=sample_size, replace=False)
    D0_sample = D0[ix,:-1]
        
        
    F = EiForest(D0_sample, ntrees=numiTrees, sample_size=sample_size,
                                         threshold=pcb_param_list['anomaly_threshold_eif'], ExtensionLevel=int(
                                         pcb_param_list['extension_level'] * data.shape[1] - 2))        
    F.ensemble_fit(D0_sample)  # EiForest model trained
    ...
    print(PCBIF_time, AAR(D0_ascore),AAR(deltaD_ascore))
